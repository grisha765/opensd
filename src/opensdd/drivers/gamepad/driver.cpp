...
void Drivers::Gamepad::Driver::UpdateState( v100::PackedInputDataReport* pIr )
{
    ...
    mState.pad.l.sx             = ((double)pIr->l_pad_x + PAD_X_MAX) * mState.pad.l.sens_x;
    mState.pad.l.sy             = ((double)pIr->l_pad_y * -1.0 + PAD_Y_MIN) * mState.pad.l.sens_y;
    ...
    mState.pad.r.sx             = ((double)pIr->r_pad_x + PAD_X_MAX) * mState.pad.r.sens_x;
    mState.pad.r.sy             = ((double)pIr->r_pad_y * -1.0 + PAD_Y_MIN) * mState.pad.r.sens_y;
    ...
    else
    {
        ...
        mState.pad.l.dx *= mState.pad.l.decay_x;
        mState.pad.l.dy *= mState.pad.l.decay_y;
    }
    ...
    {
        // Delta decay / inertia
        mState.pad.r.dx *= mState.pad.r.decay_x;
        mState.pad.r.dy *= mState.pad.r.decay_y;
    }
    ...
    switch (mode)
    {
        ...
        case BindMode::BUTTON:
            switch (bind.ev_type)
            {
                ...
                case EV_REL:
                    ...
                    if (state) 
                        device->UpdateRel( bind.ev_code, (bind.dir) ? 1 : -1 );  // TODO: Button -> REL event axis scaling?  Maybe?
                break;
        ...
        case BindMode::RELATIVE:
            switch (bind.ev_type)
            {
                ...
                case EV_REL:
                    device->UpdateRel( bind.ev_code, state, bind.scale );
                break;
...
int Drivers::Gamepad::Driver::SetProfile( const Drivers::Gamepad::Profile& rProf )
{
    ...
    SetPadSensitivity( AxisEnum::L_PAD_X, rProf.pad.l.sens.x );
    SetPadSensitivity( AxisEnum::L_PAD_Y, rProf.pad.l.sens.y );
    SetPadSensitivity( AxisEnum::R_PAD_X, rProf.pad.r.sens.x );
    SetPadSensitivity( AxisEnum::R_PAD_Y, rProf.pad.r.sens.y );
        
    // Set touchpad intertial decay
    SetPadInertiaDecay( AxisEnum::L_PAD_X, rProf.pad.l.decay.x );
    SetPadInertiaDecay( AxisEnum::L_PAD_Y, rProf.pad.l.decay.y );
    SetPadInertiaDecay( AxisEnum::R_PAD_X, rProf.pad.r.decay.x );
    SetPadInertiaDecay( AxisEnum::R_PAD_Y, rProf.pad.r.decay.y );
...
void Drivers::Gamepad::Driver::SetDeadzone( AxisEnum ax, double dz )
{
    ...
    switch (ax)
    {
        ...
        default:
            // Unhandled enums
            gLog.Write( Log::WARN, FUNC_NAME, "Invalid axis enum." );
        break;
    }
}
...
void Drivers::Gamepad::Driver::SetPadSensitivity( AxisEnum pad, double value )
{
    // Precalculated base value from hid reports
    double bx = v100::PAD_X_SENS_MULT;
    double by = v100::PAD_Y_SENS_MULT;
    
    switch (pad)
    {
        case AxisEnum::L_PAD:
            mState.pad.l.sens_x = bx * value;
            mState.pad.l.sens_y = by * value;
        break;

        case AxisEnum::L_PAD_X:
            mState.pad.l.sens_x = bx * value;
        break;

        case AxisEnum::L_PAD_Y:
            mState.pad.l.sens_y = by * value;
        break;
        
        case AxisEnum::R_PAD:
            mState.pad.r.sens_x = bx * value;
            mState.pad.r.sens_y = by * value;
        break;
        
        case AxisEnum::R_PAD_X:
            mState.pad.r.sens_x = bx * value;
        break;

        case AxisEnum::R_PAD_Y:
            mState.pad.r.sens_y = by * value;
        break;
        
        default:
            // Invalid pad ID
            gLog.Write( Log::WARN, FUNC_NAME, "Invalid touchpad ID." );
        break;
    }
}
...
void Drivers::Gamepad::Driver::SetPadInertiaDecay( AxisEnum pad, double value )
{
    // Clamp 0.0 - 1.0
    if (value < 0)
        value = 0;
    ...
    {
        case AxisEnum::L_PAD:
            mState.pad.l.decay_x = 1.0 - value;
            mState.pad.l.decay_y = 1.0 - value;
        break;
        ...
        case AxisEnum::R_PAD:
            mState.pad.r.decay_x = 1.0 - value;
            mState.pad.r.decay_y = 1.0 - value;
        break;
